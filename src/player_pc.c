...
static const struct ListMenuTemplate gUnknown_085DFF44 = //item storage list
{
    .items = NULL,
...
static const struct WindowTemplate gUnknown_085DFF5C[5] = //ItemStorageWindowTemplates
{
    {//item list window
        .bg = 0,
        .tilemapLeft = 16,
...
static void sub_816BC58(void) //remove item storage windows and free struct
{
    u32 i;
...
static void sub_816BCC4(u8 a) //remove window
{
    u8 *windowIdLoc = &(gUnknown_0203BCC4->windowIds[a]);
    if (*windowIdLoc != 0xFF)
    {
...
static void sub_816C060(u16 itemId) //item storage print item icon
{
    u8 spriteId;
...
static void sub_816C0C8(void) //remove item storage selected item icon
{
    u8* spriteIdLoc = &(gUnknown_0203BCC4->spriteId);
    if (*spriteIdLoc != 0xFF)
    {
...
static void sub_816C110(void) //calculate used pc item slots
{
    CompactPCItems();
    sub_812220C(gSaveBlock1Ptr->pcItems, 50, &(playerPCItemPageInfo.pageItems), &(playerPCItemPageInfo.count), 0x8);
}

static void sub_816C140(void) //calculate cursor position from offset, maxShownItems and numItems 
{
