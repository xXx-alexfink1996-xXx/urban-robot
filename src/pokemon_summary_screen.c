...
void ShowPokemonSummaryScreen(u8 mode, void *mons, u8 monIndex, u8 maxMonIndex, void (*callback)(void))
{
    ...
    if (gMonSpritesGfxPtr == NULL)
        CreateMonSpritesGfxManager();	//Summary Screen for EWRAM DATA
...
static void CloseSummaryScreen(u8 taskId)
{
    if (MenuHelpers_ShouldWaitForLinkRecv() != TRUE && !gPaletteFade.active)
    {
        ...
        if (gMonSpritesGfxPtr == NULL)
            DestroyMonSpritesGfxManager();	//Summary Screen for EWRAM DATA
        FreeSummaryScreen();
        DestroyTask(taskId);
    }
}
...
static u8 LoadMonGfxAndSprite(struct Pokemon *mon, s16 *state)
{
    ...
    case 0:
        ...
        else
        {
            ...
            {
                HandleLoadSpecialPokePic(TRUE,
                                         MonSpritesGfxManager_GetSpritePtr(B_POSITION_OPPONENT_LEFT),	//Summary Screen for EWRAM DATA
                                         summary->species2,
                                         summary->pid);
            }
        }
